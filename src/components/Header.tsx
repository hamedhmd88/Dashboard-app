...
function Header() {
  ...
  const [messages, setMessages] = useState([
    ...
    { sender: "سارا محمدی", preview: "به‌روزرسانی وضعیت...", date: "۱۴۰۲/۰۸/۰۷" },
  ...
  const dropdownContent = (
    <div className={`p-4 overflow-y-auto ${isMobile ? 'h-full' : ''}`}>
      <h3 className="text-sm font-semibold mb-2">پیام‌های جدید ({messages.length})</h3>
      {messages.length === 0 ? (
        <p className="text-sm text-[var(--text-secondary)]">هیچ پیامی وجود ندارد</p>
      ) : (
        <>
          {(showAll ? messages : messages.slice(0, 2)).map((msg, index) => (
            <div key={index} className="py-2 border-b last:border-b-0 flex justify-between items-start">
              <div>
                <p className="text-sm font-medium">{msg.sender} - {msg.date}</p>
                <p className="text-xs text-[var(--text-secondary)]">{msg.preview}</p>
              </div>
              <Trash 
          ...
          <div className="mt-2 text-xs text-blue-500 cursor-pointer flex items-center justify-center" onClick={() => setShowAll(!showAll)}>
            {showAll ? (
              <>
  ...
  return (
    ...
    isMobile ? (
      ...
      >
        <div 
          className={`fixed top-0 left-0 h-full w-4/5 bg-[var(--component-bg)] border-r border-[var(--border)] shadow-lg transition-transform duration-300 transform ${isDropdownOpen ? 'translate-x-0' : '-translate-x-full'}`}
