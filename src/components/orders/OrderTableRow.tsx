  ...
  return (
    <motion.tr
      ...
      className={`flex flex-col md:table-row mb-4 md:mb-0 border-b md:border-b-0 border-gray-700 md:border-none p-2 md:p-0  
        ${editingRow === order.id ? "bg-[#2f2f2f] ring-gray-500" : ""}`}
    >
      {/* Mobile view */}
        ...
        <div className="mt-2 text-base text-gray-300 text-right">
          ...
          <div>وضعیت: {editingRow === order.id ? (
            <select
              ...
              className="bg-[#0A0A0A] text-white border border-gray-400 text-xs mx-1"
            >
            ...
            <span className={`text-white px-2 py-1 rounded ${getStatusClass(order.status || "")}`}>{order.status}</span>
          )}</div>
